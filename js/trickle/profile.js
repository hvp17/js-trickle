...
$(document).ready(function() {
  $.ajax({
    ...
    $.each(jData, function(i, item) {
      $("img#imgUser").attr(
        "src",
        "images/users/" + escapeHtml(jData[i]["img"])
      );
    });
  });
});
...
$(document).ready(function() {
  $.ajax({
    ...
    if (dates || titles || qIds) {
      ...
      $.each(dates, function(i, item) {
        $("#question-container tbody").append(`
            <tr>
            <td>
                <span class="float-right font-weight-bold" id="${escapeHtml(
                  qIds[i]
                )}">${escapeHtml(dates[i])}</span> ${escapeHtml(titles[i])}
            </td>
            </tr>`);
      });
    } else {
    ...
    $("#frmEdit").append(`
            ...
            <div class="col-lg-9">
                <input maxlength="25" class="form-control" type="text" name="txtUsername" value="${escapeHtml(
                  jData[0]["username"]
                )}">
            </div>
        </div>
        <div class="form-group row">
            ...
            <div class="col-lg-9">
                <input maxlength="20" class="form-control txtEmail" type="email" name="txtEmail" value="${escapeHtml(
                  jData[0]["email"]
                )}">
            </div>
        </div>
        <div class="form-group row">
            ...
            <div class="col-lg-9">
                <input minlength="8" maxlength="20" class="form-control" type="password" name="txtPassword" placeholder="(if empty, the value will be your previous password)" value="">
            </div>
        </div>
        <div class="form-group row">
            <label class="col-lg-3 col-form-label form-control-label">Confirm password</label>
            <div class="col-lg-9">
                <input minlength="8" maxlength="20" class="form-control" type="password" name="txtConfirmPassword" placeholder="(if empty, the value will be your previous password)" value="">
            </div>
        </div>
        <div class="form-group row">
    ...
    $.each(jData, function(i, item) {
      $("#answer-container tbody").append(`
               ...
               <td>
                   <span class="float-right font-weight-bold" id="${escapeHtml(
                     jData[i]["id"]
                   )}">${escapeHtml(jData[i]["date"])}</span> ${escapeHtml(jData[i]["answer"])}
               </td>
               </tr>`);
    });
  });
